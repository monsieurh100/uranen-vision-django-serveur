...
# from myapp.serializers import LogEntrySerialize
...
class ClientViewSet(viewsets.ModelViewSet):
    ...
    def medecin_list(self,request):
        ...
        if int(site_id)>0:
            client=Client.objects.values("id","site__name","nom_client","sexe","sexe__libelle_sexe","age","consultation","prescription","ordonnance_lunette").filter(site_id=site_id).order_by("-id")
        elif int(site_id)==0:
                client=Client.objects.values("id","site__name","nom_client","sexe","sexe__libelle_sexe","age","consultation","prescription","ordonnance_lunette").order_by("-id")
    ...
    def by_site(self, request):
        site_id=request.query_params.get('site_id', None)
        if int(site_id)>0 :
            clients =Client.objects.values("site__name","sexe__libelle_sexe","nom_client","numero_client","adresse","id").filter(site=site_id).order_by('-id')
        elif int(site_id)==0:
...
class VenteViewSet(viewsets.ModelViewSet):
    ...
    authentication_classes = [TokenAuthentication]
    def list(self,request, *args, **kwargs):
        Stocks=Vente.objects.values("dates_stock","client__nom_client","client__numero_client","article__libelle_article","montant_fixe","prix_convenu","remise","site__name")
        return Response(Stocks)
    @action(detail=False, methods=['get'])
    def by_site(self, request):
        site_id=request.query_params.get('site_id', None)
        if int(site_id)>0:
            Stocks =Vente.objects.values("site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise").order_by('-id').filter(site=site_id)
        elif int(site_id)==0:
            Stocks =Vente.objects.values("site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise").order_by('-id')
        return Response(Stocks)

    @action(detail=False, methods=['get'])
    def by_client(self, request):
        client_id=request.query_params.get('client_id', None)
        stock= Vente.objects.values("dates_stock","numero_facture","article__libelle_article","quantite","prix_convenu").filter(client=client_id)
        return Response(stock)

    @action(detail=False, methods=['get'])
    def numero_facture(self, request):
        numero_facture=request.query_params.get('numero_facture', None)
        stock= Vente.objects.values("dates_stock","client__nom_client","client","montant_fixe","remise","numero_facture","article__libelle_article","quantite","prix_convenu").filter(numero_facture=numero_facture)
        return Response(stock)
    ...
    def by_site_and_date(self, request):
        ...
        try:    
            ...
            site_id=request.query_params.get('site_id', None)
            if int(site_id)>0:
                Stocks =Vente.objects.values("site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise").order_by('-id').filter(dates_stock__range=(start_date, end_date)).filter(site=site_id).order_by('-id')
            elif int(site_id)==0:
                Stocks =Vente.objects.values("site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise").order_by('-id').filter(dates_stock__range=(start_date, end_date)).order_by('-id')    
        except:
            print("erreur")
            pass
        return Response(Stocks)
    ...
    def facture(self,request):
        site_id=request.query_params.get('site_id', None)
        facture=Vente.objects.values("numero_facture").filter(site=site_id).order_by("-id").distinct()
        return Response(facture)

    @action(detail=False, methods=['get'])
    def by_client(self,request):
        client_id=request.query_params.get("client_id",None)
        stock=Vente.objects.values("id","client","dates_stock","client__nom_client","article","article__libelle_article","quantite","montant_fixe","prix_convenu",'remise').filter(client=client_id)
        return Response(stock)
...
class SortieViewSet(viewsets.ModelViewSet):
    ...
    def by_site(self, request):
        site_id=request.query_params.get('site_id', None)
        if int(site_id)>0:
            sortie=Sortie.objects.values("id","dates_sortie","depense__initule_depense","montant_depense","user","site").filter(site=site_id).order_by("-id")
        elif int(site_id)==0:
            sortie=Sortie.objects.values("id","dates_sortie","depense__initule_depense","montant_depense","user","site").order_by("-id")
        return Response(sortie)
    @action(detail=False, methods=['get'])
    def by_site_and_date(self, request):
        sortie={}
...
class AutreStockViewSet(viewsets.ModelViewSet):
    ...
    def by_site(self,request):
        ...
        entree_stock={}
        if int(site_id)>0:
            entree_stock=stock_autre.objects.values("id","dates_entree","type_operation__libelle_type","article__libelle_article","user__username","site","quantite","prix_unitaire").filter(site=site_id).order_by("-id")
        elif int(site_id)==0:
            entree_stock=stock_autre.objects.values("id","dates_entree","type_operation__libelle_type","article__libelle_article","user__username","site","quantite","prix_unitaire").order_by("-id")
...
class ManagementViewSet(viewsets.ModelViewSet):
    ...
    def vente_bysite(self,request):
        ...
        vente={}
        if site_id>0:
            vente=Article.objects.values("libelle_article").filter(vente__site=site_id).annotate(total_vente=models.Sum("vente__total"))
        elif site_id==0:
            vente=Article.objects.values("libelle_article").annotate(total_vente=models.Sum("vente__total"))    
    ...
    def vente_by_site_and_date(self,request):
        ...
        try:    
            ...
            site_id=request.query_params.get('site_id', None)
            if int(site_id)>0:
                vente=Article.objects.values("libelle_article").filter(vente__site=site_id,vente__dates_stock__range=(start_date, end_date)).annotate(total_vente=models.Sum("vente__total"))
            elif int(site_id)==0:
                vente=Article.objects.values("libelle_article").filter(vente__dates_stock__range=(start_date, end_date)).annotate(total_vente=models.Sum("vente__total"))        
        except:
            pass
        return Response(vente)
    ...
    def perception_bysite(self,request):
        site_id=request.query_params.get('site_id', None)
        if site_id>0:
            vente = Client.objects.values("id","nom_client").filter(vente__site=site_id).annotate(total_ventes=models.Sum('vente__total'))
            ...
            vente= Client.objects.values("id","nom_client").annotate(total_ventes=models.Sum('vente__total'))
    ...
    def perception_bysite_and_date(self,request):
        ...
        try:  
            ...
            site_id=request.query_params.get('site_id', None)
            if int(site_id)>0:
                vente= Client.objects.values("id","nom_client").filter(vente__site=site_id, vente__dates_stock__range=(start_date, end_date) ).annotate(total_ventes=models.Sum('vente__total'))
                ...
                vente= Client.objects.values("id","nom_client").filter(vente__dates_stock__range=(start_date, end_date) ).annotate(total_ventes=models.Sum('vente__total'))
    ...
    def reste_bysite(self,request):
        site_id=request.query_params.get('site_id', None)
        if int(site_id)>0:
            vente= Client.objects.values("id","nom_client").filter(vente__site=site_id).aggregate(total_ventes=models.Sum('vente__total'))
            ...
            vente= Client.objects.values("id","nom_client").aggregate(total_ventes=models.Sum('vente__total'))
    ...
    def History_vente_by_site(self,request):
        site_id=request.query_params.get('site_id', None)
        if int(site_id)>0:
            historique = Vente.history.values("history_date","history_type","history_user__username","site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise").filter(site=site_id)
        elif int(site_id)==0:
            historique = Vente.history.values("history_date","history_type","history_user__username","site__name","dates_stock","numero_facture","client__nom_client","article__libelle_article","montant_fixe","prix_convenu","quantite","remise")  
    ...
    def user_list(self,request):
        site_id=request.query_params.get('site_id',None)
        if int(site_id)>0:
            user=User.objects.values("id","username","groups__name","is_active").filter(groups=site_id).order_by('-is_active', 'username')
        elif int(site_id)==0:
            user=User.objects.values("id","username","groups__name","is_active").order_by('-is_active', 'username')
